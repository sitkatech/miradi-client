...
public class MigrationTo23 extends AbstractMigration
{
	...
	private class WorkPlanBudgetColumnCodeListVisitor extends AbstractMigrationORefVisitor
	{
		...
		private MigrationResult removeFields(RawObject rawObject) throws Exception
		{
			...
			if (tableSettingsMap.contains(WORK_PLAN_BUDGET_COLUMNS_CODELIST_KEY))
			{
				...
				if (workPlanBudgetColumnCodeList.contains(META_PLANNED_WHO_TOTAL))
					workPlanBudgetColumnCodeList.removeCode(META_PLANNED_WHO_TOTAL);
				if (workPlanBudgetColumnCodeList.contains(META_PLANNED_WHEN_TOTAL))
					workPlanBudgetColumnCodeList.removeCode(META_PLANNED_WHEN_TOTAL);
				if (workPlanBudgetColumnCodeList.contains(META_ASSIGNED_WHO_TOTAL))
	...
	public static final String META_PLANNED_WHEN_TOTAL = "PlannedEffortDatesTotal";
