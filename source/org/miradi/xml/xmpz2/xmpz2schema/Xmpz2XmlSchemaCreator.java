...
public class Xmpz2XmlSchemaCreator implements Xmpz2XmlConstants
{
	...
	{
		getSchemaWriter().defineAlias(createElementName(elementName), "element miradi:" + elementName);
		getSchemaWriter().startBlock();
	...
	private void writeTimePeriodCostsElement()
	{
		...
		getSchemaWriter().printlnIndented(getSchemaWriter().createOptionalSchemaElementWithAnd(CALCULATED_WHO, getSchemaWriter().createRequiredDotElement("ResourceId")));
	...
	private void writeWorkUnitsEntryElement()
	{
		...
		getSchemaWriter().printlnIndented(getSchemaWriter().createOptionalSchemaElement(WORK_UNITS_ENTRY + DETAILS, getSchemaWriter().createDotElement(DATE_UNIT_WORK_UNITS) + "*"));
		getSchemaWriter().endBlock();
	}
