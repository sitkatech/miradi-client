...
public class CountriesQuestion extends TwoLevelQuestion
{
	...
	private class CountriesSorter implements Comparator<ChoiceItem>
	{
		public int compare(ChoiceItem choiceItem1, ChoiceItem choiceItem2)
		{
			...
			if (isOther(choiceItem1))
				return -1;
			
			if (isOther(choiceItem2))
				return 1;
			
			if (isFictitious(choiceItem1))
				return -1;
			
			if (isFictitious(choiceItem2))
				return 1;
		...
		{
			return choiceItem.isSameCode(CODE_FOR_OTHER);
		}

		private boolean isFictitious(ChoiceItem choiceItem)
		{
			return choiceItem.isSameCode(CODE_FOR_FICTITIOUS);
		}
	}
	
	private static final String CODE_FOR_FICTITIOUS = "XXX";
	private static final String CODE_FOR_OTHER = "ZZZ";
}
