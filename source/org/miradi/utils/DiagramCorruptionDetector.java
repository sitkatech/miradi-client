	...
	protected static Vector<String> getCorruptedDiagramLinksErrorMessages(Project project, DiagramObject diagramObject)
	{
		...
		for (int index = 0; index < diagramLinkRefs.size(); ++index)
		{
			...
			{	
				...
				errorMessages.add(EAM.substituteSingleString(EAM.text("Found null diagramLink.  Diagram = %s"), diagramName));
				continue;
			}
			...
			if (fromDiagramFactor == null || toDiagramFactor == null)
			{
				...
				errorMessages.add(EAM.substituteSingleString(EAM.text("Found null from or to for diagram link.  Diagram = %s"), diagramName));
				continue;
			}
			...
			if (fromDiagramFactor.getWrappedFactor() == null || toDiagramFactor.getWrappedFactor() == null)
			{
				...
				errorMessages.add(EAM.substituteSingleString(EAM.text("Found null from wrapped factor or to wrapped factor from diagram link.  Diagram = %s"), diagramName));
				continue;
			}
		}
	...
	protected static Vector<String> getCorruptedDiagramFactorErrorMessages(Project project, DiagramObject diagramObject)
	{
		...
		for (int index = 0; index < diagramFactorRefs.size(); ++index)
		{
			...
			if (factor == null)
			{
				...
				errorMessages.add(EAM.substituteSingleString(EAM.text("Found null underlying factor  .Diagram = %s"), diagramName));
				continue;
			}
