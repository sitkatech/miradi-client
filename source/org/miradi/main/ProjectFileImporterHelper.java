...
public class ProjectFileImporterHelper
{
	private ProjectFileImporterHelper(MainWindow mainWindowToUse)
	{
	...
	public static void importIfRequested(MainWindow mainWindowToUse, String[] commandLineArgs) throws Exception
	{
		try
		{
			ProjectFileImporterHelper helper = new ProjectFileImporterHelper(mainWindowToUse);
		...
		{
			AbstractBaseProjectImporter.handleImportException(e);
		}
	}
	...
	{
		AbstractBaseProjectImporter importer = createImporter(projectFileToImport);
	...
	private AbstractBaseProjectImporter createImporter(File projectFile) throws ZipException, Exception
	{
		try
		{
			MiradiZipFile zipFile = new MiradiZipFile(projectFile);
			if (isXmpz2(projectFile))
				return new Xmpz2ProjectImporter(getMainWindow(), false);
	...
	public static boolean isXmpz2(File projectFile) throws Exception
	{
	...
	private static boolean isMpz(MiradiZipFile zipFile)
	{
		return containsFile(zipFile, ProjectFileImporterHelper.PROJECTINFO_FILE);
	}

	private static boolean containsFile(MiradiZipFile zipFile, final String fileToFind)
	{
