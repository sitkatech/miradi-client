...
public class TestMigrationTo27 extends AbstractTestMigration
{
	...
	public void testChildTasksMovedByForwardMigration() throws Exception
	{
		...
		for(ORef taskRef : taskPool.keySet())
		{
			...
			{
				IdList childTaskIdList = new IdList(TaskSchema.getObjectType(), rawTask.get(MigrationTo27.TAG_SUBTASK_IDS));
			...
			{
				IdList grandchildTaskIdList = new IdList(TaskSchema.getObjectType(), rawTask.get(MigrationTo27.TAG_SUBTASK_IDS));
				assertEquals(grandchildTaskIdList, new IdList(ObjectType.TASK, new BaseId[]{}));
			}
			if (taskId.equals(grandchildTask.getId()))
			{
				assertFalse(rawTask.containsKey(MigrationTo27.TAG_SUBTASK_IDS));
			}
		}
	}
