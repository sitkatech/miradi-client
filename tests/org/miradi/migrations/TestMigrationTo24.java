...
public class TestMigrationTo24 extends AbstractTestMigration
{
	...
	public void testFieldsRemovedAfterReverseMigration() throws Exception
	{
		...
		assertFalse("Field should have been removed during reverse migration?", rawMetadata.containsKey(MigrationTo24.TAG_DAY_COLUMNS_VISIBILITY));
	}
	...
	public void testFieldAddedAfterForwardMigrationNoAssignmentsHideDays() throws Exception
	{
		...
		RawObject rawMetadata = rawProject.findObject(metadataRef);
		assertNotNull(rawMetadata);
		assertEquals("Field should have been added during forward migration with (default) hide days setting?", rawMetadata.getData(MigrationTo24.TAG_DAY_COLUMNS_VISIBILITY), MigrationTo24.HIDE_DAY_COLUMNS_CODE);
	}
	...
	public void testFieldAddedAfterForwardMigrationResourceAssignmentsWithDaysShowDays() throws Exception
	{
		...
		RawObject rawMetadata = rawProject.findObject(metadataRef);
		assertNotNull(rawMetadata);
		assertEquals("Field should have been added during forward migration with show days setting?", rawMetadata.getData(MigrationTo24.TAG_DAY_COLUMNS_VISIBILITY), MigrationTo24.SHOW_DAY_COLUMNS_CODE);
	}
	...
	private void verifyAssignmentsHaveNoDayData(RawProject rawProject, int assignmentType) throws Exception
	{
		...
		if (assignmentPool != null)
		{
			for(ORef ref : assignmentPool.keySet())
			{
				...
				if (rawObject != null && rawObject.containsKey(MigrationTo24.TAG_DATEUNIT_EFFORTS))
				{
					DateUnitEffortList rawObjectDateUnitEffortList = new DateUnitEffortList(rawObject.get(MigrationTo24.TAG_DATEUNIT_EFFORTS));
